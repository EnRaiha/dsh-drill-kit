...
|---|---|---|---|
| 1 | **Localize** — cari tapak sebenar dari stack trace/log | `drill_record kind=locate` dengan `files` (path:line) | c2g / codegraph / read |
| 2 | **Blast radius** — pemanggil, callee, jenis terjejas | `drill_record kind=blast` dengan `files`/`symbols` | c2g callers/impact |
| 3 | **Edge cases** — invarian yang mesti gagal | `drill_record kind=edge` dengan `text` | fikir, senaraikan |
...
| 6 | **Review 2** — auditor segar, read-only | `drill_record kind=review` verdict PASS, blockers 0 | `drill_review` / subagent |
...
3. **Jangan lapor siap semasa gate terbuka.** Panggil `drill_gate`; kalau `ready=false`, sebut gate mana yang tinggal, jangan ganti dengan ayat "should work".
4. **Review 2 dalam sesi segar.** Auditor tak boleh jadi penulis kod. Read-only, tiada edit/commit/push.
5. **Satu issue satu PR.** Badan PR: defect + fix + cara uji + bukti regresi. `Fixes #<n>`. Nombor issue tak muncul dalam kod atau mesej commit.
6. **Env repo dihormati.** Guna `~/scripts/nodedb-cargo.sh <worktree> …` (target per-worktree, sccache, `RUST_MIN_STACK`), bukan `cargo` kosong.
