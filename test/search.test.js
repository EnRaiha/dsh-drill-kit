...
import { definitionPattern, parseJsonLine, resolveBin, resolveEngine, searchText } from '../lib/search.js'
...
test('resolveEngine prefers tgrep only when the root carries an index', () => {
  const auto = resolveEngine({ engine: 'auto', root: repo })
  assert.equal(auto.engine, 'rg', 'no .tgrep index in this temp repo')

  mkdirSync(join(repo, '.tgrep'), { recursive: true })
  const indexed = resolveEngine({ engine: 'auto', root: repo, tgrepBin: resolveBin('tgrep') ?? 'tgrep' })
  if (resolveBin('tgrep') === null) {
    assert.equal(indexed.engine, 'rg', 'tgrep is not installed here, so auto falls back to rg')
  } else {
    assert.equal(indexed.engine, 'tgrep', 'an indexed root switches auto to tgrep')
  }
