...
export function discoverDb(root, cacheDir = join(homedir(), '.cache', 'code2graph', 'projects'), sqliteBin = 'sqlite3') {
  ...
  for (const entry of readdirSync(cacheDir)) {
    ...
    // several caches can match one path. The most specific root wins, and a
    // cache with an active snapshot beats one without: a stale fixture rooted at
    // /tmp must not shadow the cache for the repository actually being drilled.
    const score = [canonical.length, snapshotId === null ? 0 : 1]
  ...
  return best === null ? null : { db: best.db, schemaVersion: best.schemaVersion, snapshotId: best.snapshotId }
}
